...
if (hlm_use_ed_prescribed_phys .eq. ifalse) then
    ...
    if ( hlm_model_day > 90.0_r8 ) then         !!!! temporary shutoff of hydraulic failure mortality until after temp_delay days are exceeded
 ...
 subroutine Mortality_Derivative( currentSite, currentCohort, bc_in, frac_site_primary, &
    ...
    call LoggingMortality_frac(ipft, currentCohort%dbh, currentCohort%canopy_layer, &
                               ...
                               currentCohort%patchptr%coage_max, & ! currentCohort%patchptr%age_since_anthro_disturbance, & [!Jstenzel edit #2]  to use patch max cohort age, not age since anthro disturbance
