...
module FatesAllometryMod
  ...
  public :: set_root_fraction  ! Generic wrapper to calculate normalized
                               ! root profiles
  public :: set_root_fraction_dbh  ! [JStenzel] Calculate pft root profiles with depths
                                   ! that increase with tree dbh.
...
contains
  ...
  subroutine carea_allom(dbh,nplant,patch_spread,ipft,crowndamage,c_area,inverse) ![JStenzel]
     ...
     associate( dbh_maxh    => prt_params%allom_dbh_maxheight(ipft), &
       ...
       case(1)
          ...
          call carea_2pwr(dbh_eff,patch_spread,d2bl_p2,d2bl_ediff,d2ca_min,d2ca_max, &
               crowndamage,c_area, do_inverse)
          capped_allom = .true.
       case(2)   ! "2par_pwr")
          call carea_2pwr(dbh,patch_spread,d2bl_p2,d2bl_ediff,d2ca_min,d2ca_max, &
               crowndamage, c_area, do_inverse)
          capped_allom = .false.
       case(3)
          dbh_eff = min(dbh,dbh_maxh)
...
!      bsap =  hbl2bsap/(la_per_sa_int + h*la_per_sa_slp) * (h/agb_fraction) * bleaf
  ...
  ! =========================================================================
  ! [JStenzel]
  ! Biomass distribution upon soil flux will  not use this subroutine yet.

  subroutine set_root_fraction_dbh(root_fraction, ft, zi, max_nlevroot, dbh)
     !
     ! !DESCRIPTION:
     !  Calculates the fractions of the root biomass in each layer for each pft.
     !  It assumes an exponential decay.  If the soil depth or max rooting depth is shallower than
     !  then exponential attenuation function, then it will normalize
     !  the profile and divide through. ![JStenzel] If the max rooting depth is shallower than
     ...
     real(r8),intent(in)    :: dbh               ! cohort dbh [cm]
     ...
     ! Edits to exponential_2p_profile_type calculation to normalize by max rooting-depth root fraction
      ...
      !root_cumulative_max = 1 - .5_r8 * exp(-a * z_fr) + exp(-b * z_fr))
      sum_rootfr = 0.0_r8

      !do lev = 1, nlevroot
      !   root_fraction(lev) = .5_r8 *  &
      !         ( exp(-a * min( z_fr,zi(lev-1)))  &
      !         + exp(-b * min( z_fr, zi(lev-1)))  &
      !         - exp(-a * min( z_fr, zi(lev)))    &
      !         - exp(-b * min( z_fr, zi(lev)))  )  !/ root_cumulative_max
      !   sum_rootfr = sum_rootfr + root_fraction(lev)
      !end do

      !Put a cap on depths at which root fractions can exist.
      do lev = 1, nlevroot
         if ( zi(lev) .le. z_fr ) then
            root_fraction(lev) = .5_r8 *  &
                  ( exp(-a * zi(lev-1))  &
                  + exp(-b * zi(lev-1))  &
                  - exp(-a * zi(lev))    &
      ...
      ! Normalize the root profile by the maximum calculated cumulative root proportion ![JStenzel]
      root_fraction(1:nlevroot) = root_fraction(1:nlevroot)/sum_rootfr
